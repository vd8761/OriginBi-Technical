...
const ReadingTaskComponent: React.FC<ReadingTaskProps> = ({ task, value = {}, onChange }) => {
    ...
    const hasPassage = Boolean(task.passage && task.passage.trim() !== "");

    const formattedPassage = hasPassage
        ? (task.passage || "").split("\\n").map((line, index) => (
    ...
    return (
        <div className={hasPassage ? `grid grid-cols-1 gap-8 ${isMaximized ? "" : "xl:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]"}` : "w-full max-w-3xl mx-auto flex flex-col gap-6"}>
            {hasPassage && (
                <motion.section 
                    layout
                    initial={false}
            ...
            <AnimatePresence>
                {(!hasPassage || !isMaximized) && (
                    <motion.section 
                        initial={{ opacity: 0, x: hasPassage ? 20 : 0 }}
                        animate={{ opacity: 1, x: 0 }}
                        exit={{ opacity: 0, x: hasPassage ? 20 : 0 }}
                        transition={{ type: "spring", damping: 25, stiffness: 200 }}
