...
interface CodingBankManagerProps {
  embedded?: boolean;
  onBack?: () => void;
}

export default function CodingBankManager({ embedded = false, onBack }: CodingBankManagerProps) {
  // Call register page hook conditionally (non-react hook cannot be conditional, but useRegisterAdminPage matches)
  useRegisterAdminPage(
    embedded
      ? { eyebrow: "Workspace", title: "Coding Assessment", breadcrumb: [{ label: "Assessments", onClick: onBack }, { label: "Coding Assessment" }] }
      : {
  ...
  return (
    <div className={embedded ? "w-full" : "admin-page"}>
      {embedded ? (
        <div className="flex flex-col sm:flex-row justify-between gap-4 items-start sm:items-center border-b border-slate-200 dark:border-white/5 pb-4 mb-4">
          <div className="flex items-center gap-3">
            {onBack && (
              <button
                type="button"
                onClick={onBack}
                className="p-2 -ml-2 rounded-lg hover:bg-slate-100 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400 transition"
            ...
            <h3 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <Code2 className="text-[#f59e0b] w-5 h-5" />
              Coding & Exam-Engine Bank
            </h3>
          </div>
