...
function AdaptiveRoleContent() {
  ...
  const handleComplete = (r: AdaptiveFinalReport) => {
    try {
      ...
      localStorage.setItem("adaptiveRoleResults", JSON.stringify({
      ...
      console.error("[AdaptiveRole] handleComplete error:", err);
    }
    setReport(r);
  };
    ...
    return (
      <AdaptiveReportV2
        ...
        onClose={() => {
          ...
          else router.push("/dashboard?completed=role");
        }}
      />
    );
  }
...
export default function AdaptiveRolePage() {
  return (
