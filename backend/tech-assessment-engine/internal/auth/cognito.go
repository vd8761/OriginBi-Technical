...
// Cognito: signature, expiry, issuer, client_id, and token_use=access.
//
// This mirrors exam-engine/internal/auth/cognito.go. The two services are
// separate Go modules so the code cannot be imported directly; keep the two
// verifiers in sync when either changes.
package auth
...
// NewCognitoVerifier primes the JWKS cache. Returns an error if the well-known
// URL is unreachable, so misconfiguration fails fast on boot rather than
// silently degrading to unauthenticated access.
...
// Verify parses and validates a Cognito ACCESS token (not an id token).
func (c *CognitoVerifier) Verify(ctx context.Context, token string) (*CognitoClaims, error) {
	...
	// token_use must be "access" — we do not accept id tokens here.
	v, ok := tok.Get("token_use")
	if !ok {
