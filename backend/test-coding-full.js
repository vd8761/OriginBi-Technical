...
async function test() {
  ...
  try {
    ...
    const attemptResult = await client.query(
      `INSERT INTO tech_coding_attempts (assessment_id, user_id, attempt_token, status, started_at, expires_at, created_at, updated_at)
       VALUES ($1, $2, $3, 'in_progress', NOW(), NOW() + INTERVAL '90 minutes', NOW(), NOW())
       RETURNING coding_attempt_id`,
      [assessmentId, 1, 'TEST-CODE-FULL-' + Date.now()]
    );
    const attemptId = attemptResult.rows[0].coding_attempt_id;
    console.log('Attempt ID:', attemptId);

    // Insert junction records
    for (let i = 0; i < Math.min(2, questions.rows.length); i++) {
      await client.query(
    ...
    console.log('Junction records inserted');

    // Now test the EXACT query from getAttemptQuestionsByConfig
    const query = `
      SELECT aq.display_order,
             q.coding_question_id as question_id,
