...
export class AssessmentService {
  ...
  private async getAttemptQuestionsByConfig(attemptId: number, config: any, shuffleOptions: boolean, seed: string) {
    const questionRows = await this.dataSource.query(
      `SELECT q.*, aq.display_order, q.${config.idCol} as question_id, ass.difficulty_marks, ass.difficulty_negative_marks
    ...
    for (const q of questionRows) {
        ...
        const diffMarks = q.difficulty_marks ? (typeof q.difficulty_marks === 'string' ? JSON.parse(q.difficulty_marks) : q.difficulty_marks) : {};
        const diffNegMarks = q.difficulty_negative_marks ? (typeof q.difficulty_negative_marks === 'string' ? JSON.parse(q.difficulty_negative_marks) : q.difficulty_negative_marks) : {};

        const difficulty = q.difficulty || 'medium';
        const marks = Number(diffMarks[difficulty] !== undefined ? diffMarks[difficulty] : (q.marks ?? 1));
        const negativeMarks = Number(diffNegMarks[difficulty] !== undefined ? diffNegMarks[difficulty] : (q.negative_marks ?? 0));
