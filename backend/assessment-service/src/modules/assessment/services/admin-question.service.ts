...
export class AdminQuestionService {
    ...
    try {
      ...
      return await this.dataSource.query(
        `SELECT a.assessment_id, a.assessment_code, a.assessment_name, a.module_type,
          a.total_time_minutes, a.total_questions, a.question_limit, a.trial_question_limit, a.status, a.created_at,
  ...
  async updateAssessment(id: number, data: any) {
    ...
    const trialQuestionLimit = data.trialQuestionLimit !== undefined ? data.trialQuestionLimit : data.trial_question_limit;
      ...
      await queryRunner.query(
        `UPDATE tech_assessments
         SET assessment_name = COALESCE($1, assessment_name),
             ...
             trial_question_limit = COALESCE($4, trial_question_limit),
             categories = COALESCE($5::jsonb, categories),
             difficulty_marks = COALESCE($6::jsonb, difficulty_marks),
             difficulty_negative_marks = COALESCE($7::jsonb, difficulty_negative_marks),
             tab_switch_limit = COALESCE($8, tab_switch_limit),
             anti_copy_enabled = COALESCE($9, anti_copy_enabled),
             shuffle_questions = COALESCE($10, shuffle_questions),
             shuffle_options = COALESCE($11, shuffle_options),
             amount = COALESCE($12, amount),
             trial_attempts_limit = COALESCE($13, trial_attempts_limit),
             main_attempts_limit = COALESCE($14, main_attempts_limit),
             enabled_question_types = COALESCE($15::jsonb, enabled_question_types),
             proctoring_require_fullscreen = COALESCE($17, proctoring_require_fullscreen),
             fullscreen_exit_limit = COALESCE($18, fullscreen_exit_limit),
             proctoring_block_devtools = COALESCE($19, proctoring_block_devtools),
             devtools_open_limit = COALESCE($20, devtools_open_limit),
             mouse_focus_loss_limit = COALESCE($21, mouse_focus_loss_limit),
             keypress_log_enabled = COALESCE($22, keypress_log_enabled),
             require_camera_mic = COALESCE($23, require_camera_mic),
             live_proctoring_enabled = COALESCE($24, live_proctoring_enabled),
             adaptive_enabled = COALESCE($25, adaptive_enabled),
             adaptive_total_marks = COALESCE($26, adaptive_total_marks),
        ...
        [
          ...
          trialQuestionLimit !== undefined ? Number(trialQuestionLimit) : null,
