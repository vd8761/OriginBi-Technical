    ...
    for (const question of aptitudeQuestions) {
        ...
        for (const option of question.options) {
            const optionResult = await client.query(
                `INSERT INTO tech_aptitude_options (aptitude_question_id, option_text, created_at) VALUES ($1, $2, NOW()) RETURNING option_id`,
...
async function seedMNC(client: any, adminUserId: number) {
    ...
    for (const question of mncQuestions) {
        ...
        for (const option of question.options) {
            const optionResult = await client.query(
                `INSERT INTO tech_mnc_options (mnc_question_id, option_text, created_at) VALUES ($1, $2, NOW()) RETURNING option_id`,
                [questionId, option.text]
            );
            if (option.isCorrect) correctOptionId = optionResult.rows[0].option_id;
        }
...
async function seedRole(client: any, adminUserId: number) {
    ...
    for (const question of roleQuestions) {
        const questionResult = await client.query(
            `INSERT INTO tech_role_questions (assessment_id, domain, question_type, question_text, scenario_context, correct_option_id, marks, negative_marks, status, created_at, updated_at)
             VALUES ($1, $2, $3, $4, $5, NULL, $6, 0, 'active', NOW(), NOW()) RETURNING role_question_id`,
            [assessmentId, question.category, 'mcq', question.question_text, 'Workplace scenario', question.marks]
        );
        ...
        for (const option of question.options) {
            const optionResult = await client.query(
                `INSERT INTO tech_role_options (role_question_id, option_text, created_at) VALUES ($1, $2, NOW()) RETURNING option_id`,
                [questionId, option.text]
            );
            if (option.isCorrect) correctOptionId = optionResult.rows[0].option_id;
        }
...
const run = async () => {
    ...
    try {
        ...
        if (!adminUserId) throw new Error("No users found. Add a user first or set ADMIN_USER_ID.");
