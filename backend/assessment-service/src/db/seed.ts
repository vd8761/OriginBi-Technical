...
const run = async () => {
    ...
    try {
        ...
        const assessmentId = assessmentResult.rows[0].assessment_id as number;
        ...
        for (const question of questions) {
            ...
            const questionId = questionResult.rows[0].aptitude_question_id as number;
            ...
            for (const option of question.options) {
                ...
                const optionId = optionResult.rows[0].option_id as number;
                if (option.isCorrect) {
        ...
        console.error("Failed to seed data:", (error as Error).message);
        process.exit(1);
    } finally {
        client.release();
        await pool.end();
