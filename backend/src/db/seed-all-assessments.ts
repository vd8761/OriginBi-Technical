...
for (let i = 0; i < 12; i++) {
    mncQuestions.push({
        category: ["Quantitative", "Logical", "Technical", "Verbal"][i % 4] as any,
...
for (let i = 0; i < 8; i++) {
    roleQuestions.push({
        category: ["Scenarios", "Conceptual", "Situational"][i % 3] as any,
...
async function seedAptitude(client: any, adminUserId: number) {
    ...
    for (const question of aptitudeQuestions) {
        const questionResult = await client.query(
            `INSERT INTO tech_aptitude_questions
            ...
            [assessmentId, question.subcategory, question.difficulty, question.question_text, null, question.marks, question.negative_marks, question.explanation]
        );
        ...
        for (const option of question.options) {
            const optionResult = await client.query(
                `INSERT INTO tech_aptitude_options
...
async function seedCoding(client: any, adminUserId: number) {
    const assessmentResult = await client.query(
        `INSERT INTO tech_assessments
         ...
         DO UPDATE SET
             ...
             updated_at = NOW()
         RETURNING assessment_id`,
        ["TECH_CODE_001", "Coding Assessment", 90, codingProblems.length, false, false, false, 0, adminUserId]
    );
    ...
    for (const problem of codingProblems) {
        await client.query(
            `INSERT INTO tech_coding_questions
            ...
            [assessmentId, problem.title, problem.description, problem.difficulty, problem.marks, 
...
async function seedCommunication(client: any, adminUserId: number) {
    const assessmentResult = await client.query(
        `INSERT INTO tech_assessments
         ...
         DO UPDATE SET
             ...
             updated_at = NOW()
         RETURNING assessment_id`,
        ["TECH_COMM_001", "Communication Skills Assessment", 45, communicationQuestions.length, false, false, false, 0, adminUserId]
    );
    ...
    for (const question of communicationQuestions) {
        await client.query(
            `INSERT INTO tech_grammar_questions
             ...
             ON CONFLICT DO NOTHING`,
            [assessmentId, question.skill, question.difficulty, question.question_text, 
...
async function seedMNC(client: any, adminUserId: number) {
    ...
    for (const question of mncQuestions) {
        const questionResult = await client.query(
            `INSERT INTO tech_mnc_questions
                (assessment_id, category, difficulty, question_text, image_url,
                 correct_option_id, marks, negative_marks, explanation, status, created_at, updated_at)
             VALUES ($1, $2, $3, $4, $5, NULL, $6, $7, $8, 'active', NOW(), NOW())
             RETURNING mnc_question_id`,
            [assessmentId, question.category, question.difficulty, question.question_text, null, 
...
async function seedRole(client: any, adminUserId: number) {
    const assessmentResult = await client.query(
        `INSERT INTO tech_assessments
         ...
         DO UPDATE SET
             ...
             updated_at = NOW()
         RETURNING assessment_id`,
        ["TECH_ROLE_001", "Role Fit Assessment", 45, roleQuestions.length, true, true, false, 0, adminUserId]
    );
    ...
    for (const question of roleQuestions) {
        const questionResult = await client.query(
            `INSERT INTO tech_role_questions
                (assessment_id, category, difficulty, question_text, scenario_context,
                 correct_option_id, marks, negative_marks, explanation, status, created_at, updated_at)
             VALUES ($1, $2, $3, $4, $5, NULL, $6, 0, $7, 'active', NOW(), NOW())
             RETURNING role_question_id`,
            [assessmentId, question.category, question.difficulty, question.question_text, 
        ...
        if (correctOptionId) {
            await client.query(
                `UPDATE tech_role_questions
                 SET correct_option_id = $1, updated_at = NOW()
                 WHERE role_question_id = $2`,
                [correctOptionId, questionId]
            );
        }
    }
