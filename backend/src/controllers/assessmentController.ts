...
type ModuleType = "aptitude" | "grammar" | "coding" | "mnc" | "role";
...
const resolveModule = (req: Request): ModuleType | null => {
    const module = String(req.params.module || req.query.module || "coding").toLowerCase();
    return (module in moduleConfig ? (module as ModuleType) : null);
};
...
const buildCodingPrompt = (row: any) => {
...
const parseMemoryKb = (value: string | null) => {
    if (!value) return null;
    const match = value.match(/(\d+(?:\.\d+)?)/);
    if (!match) return null;
    const numeric = Number(match[1]);
    ...
    if (lower.includes("gb")) return Math.round(numeric * 1024 * 1024);
    if (lower.includes("mb")) return Math.round(numeric * 1024);
    if (lower.includes("kb")) return Math.round(numeric);
...
const normalizeAnswerPairs = (answers: any) => {
...
export const startAptitudeAttempt = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const attemptId = attemptInsert.rows[0].aptitude_attempt_id as number;

        const questionResult = await client.query(
            `SELECT aptitude_question_id, subcategory, question_text, image_url
             FROM tech_aptitude_questions
             WHERE assessment_id = $1 AND status = 'active'
             ORDER BY aptitude_question_id`,
        ...
        const optionResult = await client.query(
            `SELECT option_id, aptitude_question_id, option_text
        ...
        const optionsByQuestion = new Map<number, { id: number; text: string }[]>();
        ...
        for (let index = 0; index < orderedQuestions.length; index += 1) {
            ...
            const attemptQuestionInsert = await client.query(
                ...
                [attemptId, question.aptitude_question_id, index + 1]
            );
            const attemptQuestionId = attemptQuestionInsert.rows[0].attempt_question_id as number;
            ...
            for (let optIndex = 0; optIndex < orderedOptions.length; optIndex += 1) {
                ...
                await client.query(
                    `INSERT INTO tech_aptitude_attempt_question_options
                        (attempt_question_id, option_id, display_order)
                     VALUES ($1, $2, $3)`,
                    [attemptQuestionId, option.id, optIndex + 1]
                );
            }
...
export const getAptitudeAttempt = async (req: Request, res: Response) => {
    ...
    try {
        const attemptResult = await pool.query(
            `SELECT a.aptitude_attempt_id, a.assessment_id, a.started_at, a.expires_at,
        ...
        const questionResult = await pool.query(
            `SELECT aq.attempt_question_id, aq.display_order, q.subcategory, q.question_text, q.image_url
        ...
        const optionResult = await pool.query(
            `SELECT aqo.attempt_question_id, aqo.display_order, o.option_id, o.option_text
             ...
             ORDER BY aqo.attempt_question_id, aqo.display_order`,
            [questionResult.rows.map((row) => row.attempt_question_id)]
        );

        const optionsByAttemptQuestion = new Map<number, { id: string; text: string }[]>();
        for (const row of optionResult.rows) {
...
export const submitAptitudeAttempt = async (req: Request, res: Response) => {
    const token = req.params.token;
    ...
    try {
        ...
        const attemptResult = await client.query(
            `SELECT a.aptitude_attempt_id, a.assessment_id, a.started_at, t.negative_mark_enabled
             FROM tech_aptitude_attempts a
             JOIN tech_assessments t ON t.assessment_id = a.assessment_id
             WHERE a.attempt_token = $1`,
            [token]
        ...
        const questionResult = await client.query(
            `SELECT aq.attempt_question_id, q.correct_option_id, q.marks, q.negative_marks, q.subcategory
             FROM tech_aptitude_attempt_questions aq
             JOIN tech_aptitude_questions q ON q.aptitude_question_id = aq.aptitude_question_id
             WHERE aq.aptitude_attempt_id = $1`,
            [attempt.aptitude_attempt_id]
        ...
        const questionMap = new Map<number, typeof questionResult.rows[number]>();
        ...
        await client.query(
            `UPDATE tech_aptitude_attempts
             SET status = 'submitted',
                 ...
                 updated_at = NOW()
             WHERE aptitude_attempt_id = $1`,
            [attempt.aptitude_attempt_id, totalScore, positiveScore, negativeScore, timeTakenSeconds]
        );
    ...
    try {
        if (module === "coding") {
            const { rows } = await pool.query(
                `SELECT coding_question_id, assessment_id, difficulty, problem_title, problem_statement,
...
export const submitCode = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const assessmentResult = await client.query(
            "SELECT total_time_minutes, negative_mark_enabled FROM tech_assessments WHERE assessment_id = $1",
            [parsedAssessmentId]
        );
        const assessment = assessmentResult.rows[0];
        if (!assessment) {
        ...
        const questionResult = await client.query(
            "SELECT marks, negative_marks FROM tech_coding_questions WHERE coding_question_id = $1",
            [parsedQuestionId]
        );
        ...
        const attemptInsert = await client.query(
            `INSERT INTO tech_coding_attempts
            ...
            [parsedAssessmentId, parsedUserId, attemptToken, now, expiresAt]
        );
        ...
        const attemptQuestionInsert = await client.query(
            `INSERT INTO tech_coding_attempt_questions
        ...
        await client.query(
            `UPDATE tech_coding_attempts
             SET status = 'submitted', submitted_at = NOW(), total_score = $2,
                 positive_score = $3, negative_score = $4, updated_at = NOW()
             WHERE coding_attempt_id = $1`,
            [codingAttemptId, totalScore, scoreAwarded, negativeApplied]
        );
